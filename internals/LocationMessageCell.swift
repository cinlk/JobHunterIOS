...
@objcMembers class LocationMessageCell: UITableViewCell {
    ...
    private var mode: LocationMessage?
...
extension LocationMessageCell{
    ...
    internal func setCell(mode: MessageBoby, chatUser: HRPersonModel?){
        
        
        guard let m = mode as? LocationMessage else{
        ...
        let ckey =  NSString.init(string:  "\(String(describing: m.latitude)) + \(String(describing: m.longitude))")
        
        if let image = SingletoneClass.shared.imageCache.object(forKey: ckey){
            ...
            if (self.coordinate.coordinate.latitude != m.latitude || self.coordinate.coordinate.longitude != m.longitude ) {
                ...
                snapShotter.start(with: bgQueue, completionHandler: { [weak self] (snapshot, error) in
                    ...
                    if let snapShotImage = snapshot?.image, let coordinatePoint = snapshot?.point(for: coords), let pinImage = pin.image {
                        ...
                        SingletoneClass.shared.imageCache.setObject(mapImage!, forKey: ckey)
        ...
        if m.senderId == GlobalUserInfo.shared.getId(){
            ...
            wrapperView.frame = CGRect.init(x: GlobalConfig.ScreenW - 5 - GlobalConfig.AvatarSize.width - 5 - mapSize.width , y: 5, width: mapSize.width, height: mapSize.height)
            ...
            if let iconUrl = chatUser?.icon{
                self.avartar.kf.indicatorType = .activity
                self.avartar.kf.setImage(with: Source.network(iconUrl), placeholder: nil, options: nil, progressBlock: nil, completionHandler: nil)
            }else{
                self.avartar.image = #imageLiteral(resourceName: "default")
            }
            ...
            bubbleMaskImage = stretchImage?.resizableImage(withCapInsets: stretchInset, resizingMode: .stretch)
             ...
             wrapperView.frame = CGRect.init(x: 5 + GlobalConfig.AvatarSize.width + 5 , y: 5, width: mapSize.width, height: mapSize.height)
...
extension LocationMessageCell{
    ...
    @objc private func showMap(_ guest: UITapGestureRecognizer){
        if guest.state == .ended, let m = self.mode{
            self.navigate2Map?(m)
        }
